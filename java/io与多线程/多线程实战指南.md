...
![](/assets/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202019-05-12%20%E4%B8%8B%E5%8D%885.28.48.png)
...
3. 将线程a的Lock Record的**地址**使用CAS放到了Mark Word当中，并且把锁标志位改为00，意味着a已经获得了这个轻量级的锁了，可以继续进入临界区执行。B自旋等待
   ...
   ![](/assets/%E5%B1%8F%E5%B9%95%E5%BF%AB%E7%85%A7%202019-05-12%20%E4%B8%8B%E5%8D%885.47.21.png)
...
保障对变量操作的可见性、有序性、long/double**变量读写原子性**。但不一定保障其对volatile的赋值操作有原子性，比如：v1 = v2 + 1; v2若是共享变量就没有原子。
...
callable接口：逻辑在call方法中实现，call方法返回任务的处理结果；配合executorService使用

runnable接口：其run方法没有返回值，不能抛出异常

future接口：ThreadPoolExecutor的submit方法返回future接口类型：执行的任务的处理结果句柄

Future接口的方法：

- cancel：取消任务成功返回true，失败返回false。参数mayInterruptIfRunning表示是否允许取消正在执行却没有执行完毕的任务，如果设置true，则表示可以取消正在执行过程中的任务。如果任务已经完成，则无论mayInterruptIfRunning为true还是false，此方法肯定返回false，即如果取消已经完成的任务会返回false；如果任务正在执行，若mayInterruptIfRunning设置为true，则返回true，若mayInterruptIfRunning设置为false，则返回false；如果任务还没有执行，则无论mayInterruptIfRunning为true还是false，肯定返回true。
...
1. 因为Future只是一个接口，所以是无法直接用来创建对象使用的，FutureTask是Future唯一的实现类
2. FutureTask类实现了RunnableFuture接口，RunnableFuture继承了Runnable接口和Future接口，而FutureTask实现了RunnableFuture接口。所以它既可以作为Runnable被线程执行，又可以作为Future得到Callable的返回值。
...
![img](/assets/20180419095106513.png)
...
换句话说，线程写入，写屏障会通过类似强迫刷出处理器缓存的方式，让其他线程能够拿到最新数值。内存屏障之类只是实现 JMM 规范的技术手段，并不是规范的要求。

# 十七：一个线程两次调用 start() 方法会出现什么情况？
