...
**技术分析之Spring框架的事务管理相关的类和API**
	1. PlatformTransactionManager接口		-- 平台事务管理器.(里面的方法有：commit,rollback。真正管理事务的类)。该接口有具体的实现类，根据不同的持久层框架，需要选择不同的实现类！
	...
	6. TransactionDefinition
		...
		2. 事务的传播行为常量（不用设置，使用默认值）
			* 先解释什么是事务的传播行为：解决的是业务层之间的方法调用！！
			（如下图，在save方法中调用find。就是业务层调用业务层。save,find中都有事务，就是事务中嵌套事务）
			//保证A,B在一个事务中
...
![1541055683383](./图片/1541055683383.png)
...
-  以后可以只配置连接池，因为jdbcDaoSupport会自动通过连接池生成jdbc模板
		...
		<tx:advice id="txAdvice" transaction-manager="transactionManager">
			<tx:attributes>
				<!--
				 ...
				 -->
				<!-- 哪些方法加事务 -->
				<tx:method name="pay" propagation="REQUIRED"/> //方法名字可以有通配符
				<tx:method name="find"/>
			</tx:attributes>
		</tx:advice>
