...
示例：查询所有客户得到list。原来--存在request域，转发至jsp；现在--存在值栈里，转发到JSP页面，使用OGNL表达式从值栈中获取值	
...
**技术分析之值栈的概述**
![](图片/1540863945614.png)
	1. 问题一：什么是值栈？
		* 值栈就相当于Struts2框架的数据的中转站，向值栈存入一些数据。从值栈中获取到数据。
		* ValueStack 是 struts2 提供一个接口，实现类是 OgnlValueStack（内部有很多集合，用来存储数据） ---- 值栈对象 （OGNL是从值栈中获取数据的 ）
		* Action是多例的（即有一个请求过来，都会单独对应一个处理），对每一个请求，都会创建一个Action对象，一个ActionContext对象，代表的是Action的上下文对象，还会创建一个ValueStack对象。
		* 每个Action实例都有一个ValueStack对象 （一个请求 对应 一个ValueStack对象 ）
		* 在其中保存当前Action 对象和其他相关对象
		* Struts 框架把 ValueStack 对象保存在名为 “struts.valueStack” 的请求属性中,在request域中 （值栈对象 是 request一个属性）
...
**技术分析之向值栈中保存数据**
	...
	5. 问题五： 向值栈保存数据 （用户存电数据主要针对root栈，框架本身会将程序的数据存入context对象）-都是向栈顶中存数据
		> valueStack.push(Object obj);
			* push方法的底层调用root对象的push方法（把元素添加到0位置，然后之前在0位置的元素就被压下去了）
		
		> valueStack.set(String key, Object obj);
			* 源码获取map集合（拿的是root对象的栈顶元素）（map有可能是已经存在的，有可能是新创建的），看一下这个是不是map集合，是的话直接用。不是的话new一个map，然后把map集合push到栈顶，再把数据存入到map集合中。
		
		> 在jsp中 通过 <s:debug /> 标签查看值栈的内容（直接写）
